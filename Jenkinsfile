...
node("master"){
	checkout scm
	sh "aws s3 cp s3://dinosaurus/terraform-env/"+env.GIT_BRANCH+"/terraform-dev.tfstate terraform.tfstate"
	sh "terraform plan"
}
input "Proceed with plan execution?"
node("master"){
	sh "terraform apply"
	sh "aws s3 cp terraform.tfstate s3://dinosaurus/terraform-env/"+env.GIT_BRANCH+"/terraform-dev.tfstate"
}
